...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__indexedDB__ = __webpack_require__(4);
...
const setOneColor = (color, position) => {
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('.color' + position).css('background', color);
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('.color' + position).css('color', color);
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('.color-letter').css('background', 'none');
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('[data-id=' + position + ']').find('h3').text(color);
};
...
const toggleLocked = e => {
  const position = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(e.target).closest('[data-id]').data('id');
...
const savePalette = async e => {
  ...
  for (let i = 1; i <= 5; i++) {
    palette = Object.assign(palette, { [`color${i}`]: currentColors[i - 1] }, { project_id: __WEBPACK_IMPORTED_MODULE_0_jquery___default()('[name="current-project"]').val() });
  }
  ...
  Object(__WEBPACK_IMPORTED_MODULE_2__indexedDB__["c" /* saveOfflinePalettes */])(palette).then(() => renderPalette(palette)).catch(error => error);
...
const createProject = async e => {
  ...
  Object(__WEBPACK_IMPORTED_MODULE_2__indexedDB__["d" /* saveOfflineProjects */])({ id, name }).then(() => {
...
__WEBPACK_IMPORTED_MODULE_0_jquery___default()('[name="save-palette"]').on('click', savePalette);
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__indexedDB__ = __webpack_require__(4);


const getProjects = () => {
  return fetch('/api/v1/projects').then(response => response.json()).then(parsedResponse => parsedResponse).catch(() => {
    return Object(__WEBPACK_IMPORTED_MODULE_0__indexedDB__["b" /* loadOfflineProjects */])().then(projects => ({ projects })).catch(error => error);
  });
};
...
const getPalette = projectId => {
  return fetch(`/api/v1/projects/${projectId}/palettes`).then(response => response.json()).then(parsedResponse => parsedResponse).catch(() => {
    return Object(__WEBPACK_IMPORTED_MODULE_0__indexedDB__["a" /* loadOfflinePalettes */])(projectId).then(palettes => palettes).catch(error => error);
  });
};
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_dexie__ = __webpack_require__(5);


let db = new __WEBPACK_IMPORTED_MODULE_0_dexie__["a" /* default */]('palettePicker');
...
const loadOfflinePalettes = projectId => {
